...
export default function SuguvalHistory() {
  ...
  const { data: history, isLoading } = useQuery<HistoryEntry[]>({
    queryKey: ["/api/suguval/history", selectedMonth],
    queryFn: async () => {
      const res = await fetch(`/api/suguval/history?month=${selectedMonth}`, { credentials: "include" });
  ...
  const { data: futureItems } = useQuery<number[]>({
    queryKey: ["/api/suguval/future", selectedDate],
    queryFn: async () => {
      const res = await fetch(`/api/suguval/future?date=${selectedDate}`, { credentials: "include" });
  ...
  const addFutureItemMutation = useMutation({
    mutationFn: async ({ itemId, date }: { itemId: number; date: string }) => {
      return apiRequest("POST", "/api/suguval/future", { itemId, date });
    },
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ["/api/suguval/future", selectedDate] });
    }
  });

  const removeFutureItemMutation = useMutation({
    mutationFn: async ({ itemId, date }: { itemId: number; date: string }) => {
      return apiRequest("DELETE", "/api/suguval/future", { itemId, date });
    },
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ["/api/suguval/future", selectedDate] });
    }
            ...
            <div className="flex items-center gap-3">
              ...
              <div className="flex items-center gap-2">
                ...
                <h1 className="text-lg md:text-xl font-bold">
                  {language === "fr" ? "Calendrier" : language === "vi" ? "Lịch" : "ปฏิทิน"}
                </h1>
              </div>
            </div>
                  ...
                  return (
                    <button
                      ...
                      className={`
                        ...
                        ${hasHistory && !isCurrent ? "font-bold text-orange-400" : ""}
                      `}
